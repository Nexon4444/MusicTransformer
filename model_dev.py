...
class MusicTransformer(keras.Model):
    def __init__(self, embedding_dim=256, vocab_size=388+2, num_layer=6,
                 max_seq=2048, dropout=0.2, debug=False, loader_path=None, dist=False, training=True):
    ...
    def call(self, inputs, training=False, eval=None, src_mask=None, trg_mask=None, lookup_mask=None):
        # print(f"inputs: {inputs.shape}")
        # inputs: tuple(tf.Tensor: shape=(1, 1024)
        # inputs: (2, 1024)
        x = inputs[0]
        y = inputs[1]
        # print(f"y: {y}")
    ...
    @tf.function
    def train_step(self, inp, inp_tar, out_tar, enc_mask, tar_mask, lookup_mask, training):
